...
import random
import math
from .buffer_PPO import Trajectory
...
class Vehicle:
    ...
    def finalize_trajectory(self):
        """
        离开监控范围或仿真结束时调用，用于给 Trajectory 收尾
        """
        if self.traj.length() == 0:        # 没有任何记录则直接跳过
            return

        # 若车辆未执行计算且已离开，施加 r_S 惩罚
        penalty = None
        if (not self.is_computed) and self.is_left and self.args is not None:
            penalty = self.args.r_S

        self.traj.finalize(no_cmp_penalty=penalty)
    ...
    def update_vehicles(self):
        ...
        for vehicle_id in self.active_vehicles:
            ...
            if vehicle.time_remained <= 0:  # 如果车辆已到达路口，标记为离开
                ...
                # 为离开的车辆完成轨迹记录
                vehicle.finalize_trajectory()

            if not vehicle.is_computed:  # 如果车辆上一时隙未计算，更新上一时隙的传输数据量。此时vehicle信息中还是上一时隙的传输速率。如果上一时隙计算了，则上一时隙未传输，无新增传输量
                transmitted_bits_last_slot = min(vehicle.trans_rate * self.time_slot,
                                                 vehicle.bits_in_queue)  # 上一时隙传输的比特数
        ...
        for vehicle_id in vehicles_to_remove:
            ...
            # 将离开的车辆添加到历史车辆列表中
            self.history_vehicles.append(vehicle_id)
